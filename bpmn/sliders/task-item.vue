<template>
    <el-collapse-item title="任务监听" name="5">
...
<script>
import addUpdate from './task/add-update.vue';
export default {
    inject: {
        //来自bpmn-main组件的实例
        vm: {
    ...
    methods: {
        ...
        bpmnClick(element) {
            ...
            const list = this.getBpmn.getTaskList(element)
        ...
        refreshData(form, state) {
            ...
            if (state == 'add') {
                return this.getBpmn.createTaskEventElement(
                    this.activeElement,
                    eventType, listenerType, value,uniQueKey
                )
            }
            // this.nodeList.splice(this.index, 1)
            //修改
            return this.getBpmn.updateTaskEventElement(
                this.activeElement,
                eventType, listenerType, value, uniQueKey
            )
        },
        ...
        async removeListener(index) {
            ...
            this.getBpmn.deleteTaskEventElement(
                this.activeElement,
                uniQueKey
            )
        }
